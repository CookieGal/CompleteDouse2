...
for (var i = 0; i < array_length(l); i++)
{
	...
	switch (layer_get_name(bg_id))
	{
        ...
        case "Backgrounds_Ground3":
            ...
            break;
        case "Backgrounds_H1":
            layer_x(bg_id, offsets[i].x + (camera_pos.x * 0.93))
            layer_y(bg_id, camera_pos.y)
            break;
            break;
        case "Backgrounds_H2":
            layer_x(bg_id, offsets[i].x + (camera_pos.x * 0.85))
            layer_y(bg_id, camera_pos.y)
            break;
		...
		case "Backgrounds_scroll3":
			layer_x(bg_id, (camera_pos.x * 0.999) + (bg_scroll.x * layer_get_hspeed(bg_id)))
			layer_y(bg_id, (camera_pos.y * 0.999) + (bg_scroll.y * layer_get_vspeed(bg_id)))
			break;
		case "Foregrounds_1":
