function player_normal()
{
	...
	if global.panic.active
	{
		...
		default_move = spr_player_panicmove
		
		if global.panic.timer <= 0
		{
			default_idle = spr_player_timeidle
			default_move = spr_player_timemove
		}
	}
	if room == tower_3
	{
	...
	{
		movespeed = approach(movespeed, 4, movespeed > 4 ? 4 : 4)
		if (floor(movespeed) == 4)
			movespeed = 4
		xscale = p_move
		if particle_timer > 0
			particle_timer--
