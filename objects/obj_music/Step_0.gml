...
if isPanic
{
	if (!panic_music_initiated && !pinch_init && !lap2)
	{
		...
		audio_sound_loop_start(panic_mu, 46.81)
		audio_sound_loop_end(panic_mu, 119.4)
	}
	...
	if (global.panic.timer < pinch_point && !pinch_init && !lap2)
	{
		...
		panic_pinch_mu = scr_sound(mu_finalescape)
		audio_sound_set_track_position(panic_pinch_mu, 206.07);
...
if instance_exists(obj_pillar2)
{
	if (!panic_music_initiated && !pinch_init && !lap2)
	{
		panic_music_initiated = true
		...
		audio_stop_sound(pillar_mu)
		mu = noone
		panic_mu = scr_sound(mu_finalescape, true)
		audio_sound_loop_start(panic_mu, 75.95)
		audio_sound_loop_end(panic_mu, 206.06)
	}
}
